...
class EdgeGridTest(unittest.TestCase):
    ...
    def runTest(self):
        ...
        request = requests.Request(
            ...
            headers=headers,
            data=self.testcase["request"].get("data") if self.testcase["request"].get("data") else None,
        )

        try:
            auth_header = auth.make_auth_header(request.prepare(), self.testdata["timestamp"], self.testdata["nonce"])
