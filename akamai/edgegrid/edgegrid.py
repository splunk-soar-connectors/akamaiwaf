...
def base64_hmac_sha256(data, key):
    return base64.b64encode(hmac.new(key.encode("utf8"), data.encode("utf8"), hashlib.sha256).digest()).decode("utf8")
...
class EdgeGridAuth(AuthBase):
    ...
    def __init__(self, client_token, client_secret, access_token, headers_to_sign=None, max_body=131072):
    ...
    def canonicalize_headers(self, r):
        ...
        return "\t".join(["%s:%s" % (h, spaces_re.sub(" ", r.headers[h].strip())) for h in self.headers_to_sign if h in r.headers])
    ...
    def make_content_hash(self, r):
        ...
        if r.method == "POST" and len(prepared_body) > 0:
            ...
            if len(prepared_body) > self.max_body:
                logger.debug("data length %d is larger than maximum %d", len(prepared_body), self.max_body)
    ...
    def sign_request(self, r, timestamp, auth_header):
        return base64_hmac_sha256(self.make_data_to_sign(r, auth_header), self.make_signing_key(timestamp))
    ...
    def make_auth_header(self, r, timestamp, nonce):
        ...
        signed_auth_header = auth_header + "signature=" + self.sign_request(r, timestamp, auth_header)
    ...
    def handle_redirect(self, res, **kwargs):
        if res.is_redirect:
            ...
            res.request.headers["Authorization"] = self.make_auth_header(request_to_sign, eg_timestamp(), new_nonce())
